...
The boundary between user text and the numeric engine: correct, locale-aware parsing and formatting of PLN amounts and percentages (`parsePLN`, `formatPLN`, `formatPct`). Polish input uses the comma as the decimal separator and space/NBSP/dot as thousands separators; ambiguous input must fail loudly (`null`) rather than silently produce a wrong magnitude.
