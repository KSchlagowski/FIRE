...
Robust SVG chart scaling for negative and decimated series (viewBox clamping, min/max-preserving decimation) so `chartSVG`/`stackedBarSVG` never render points off-canvas or understate a peak dropped by sampling.
